...
const { VectorDBQAChain, LLMChain, ConversationalRetrievalQAChain} = require('langchain/chains');
...
/*const model = new ChatOpenAI({
  ...
  const prompt =
    PromptTemplate.fromTemplate(`The following is a friendly conversation between a human and an assistant. The name of AI is "DFCC chatbot". The AI is developed by ABC Company. The AI live in Sri Lanka. The AI is smart and provides lots of specific details only from its context. If answer has points, it provide those points in point format. If answer has link, it provide those links as links. If the question does not related to the context, it friendly ask from human to provide questions only related to dfcc bank. If the AI does not know the answer to a question, it truthfully says it does not know.
  ...
  const chain = VectorDBQAChain.fromLLM(model, vectorStore, prompt,memory, {
    foo: "bar",
    k: 2,
