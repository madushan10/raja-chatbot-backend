...
/** ===================================================== */
    ...
    if (resInfo.trim() === 'Name') {
      const nameTxt = "My name is DFCC GPT"
      console.log(nameTxt);
...
if (userQuestion === 'What is the 22kt Gold rate today?') {
...
else if (userQuestion === 'Where are you located at?') {
  ...
  db.query('INSERT INTO chat_bot_chats SET ?', { message_id: id, language: language, message: answer_quatation_for_custom_made_jwelry, message_sent_by: 'bot', viewed_by_admin: 'no' }, async (Err3, result3) => {
    ...
    if (resGreet.trim() === "Yes") {
        ...
        console.log(resgreet.trim());
        db.query('INSERT INTO chat_bot_chats SET ?', { message_id: id, language: language, message: resgreet.trim(), message_sent_by: 'bot', viewed_by_admin: 'no' }, async (Err3, result3) => {
          if (Err3) throw Err3;
        });
        return res.json({ status: "success", bot_reply: resgreet.trim()});
    }
    else {
